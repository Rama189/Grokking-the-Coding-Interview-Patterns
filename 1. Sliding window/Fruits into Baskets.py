...
'''
#mycode copied from previous a bit.
def fruits_into_baskets(fruits):
    ...
    for window_end in range(len(fruits)):
        ...
        if right_char not in fruit_dict:
            fruit_dict[right_char] = 0

        fruit_dict[right_char] += 1

        while len(fruit_dict) > 2:
            left_char = fruits[window_start]
            fruit_dict[left_char] -= 1
            if fruit_dict[left_char] == 0:
                del fruit_dict[left_char]
            window_start += 1
